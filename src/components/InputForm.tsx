...
const InputForm = ({ onCalculate }: InputFormProps) => {
  ...
  const SectionTitle = ({ icon: Icon, children }: { icon: React.ElementType; children: React.ReactNode }) => (
    <div className="border-t border-border pt-5 mt-5 first:border-0 first:pt-0 first:mt-0">
      <p className="text-xs uppercase tracking-wider font-extrabold text-foreground mb-3 flex items-center gap-1.5">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {coBuyers.map((cb, i) => (
              <div key={i}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5"><FieldLabel icon={CreditCard}>Deudas mensuales (€)</FieldLabel><Input type="number" placeholder="p.ej. 100" value={cb.monthlyDebts} onChange={e => updateCoBuyer(i, "monthlyDebts", e.target.value)} min={0} className="rounded-xl" /></div>
                </div>
              </div>
            ))}
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">Entrada: {100 - mortgagePercent}% del valor</p>
            </div>

            <Button type="submit" size="lg" className="w-full rounded-full font-bold text-base mt-3 h-12">
